...
def lode_scheduler(
    ...
    noise_matrix = jnp.concatenate(
        [jnp.zeros_like(current_latent)[None, :], noise_matrix], axis=0
    )
    ...
    closest_array = jnp.zeros(
        (max_candidates, 4)
    )  # stores [loss_diff, best_time, loss_final, val_final]
    ...
    for sample in range(pred_trajs.shape[0] - 1):
        ...
        time_mask = (jnp.arange(pred_traj.shape[0]) >= min_t) & (
            jnp.arange(pred_traj.shape[0]) < max_t
        )
    ...
    if verbose:
        ...
        debug.print(
            "   ▸  samples accepted:           {}/{}", closest_array.shape[0], n_samples
        )
    ...
    val_err = (
        100 * jnp.abs(best_val - validation_path[-1]) / (validation_path[-1] + eps)
    )

    if verbose:
        debug.print("   ▸  sampled loss error:         {:.3f}%", loss_err)
        debug.print("   ▸  sampled lr error:           {:.3f}%", lr_err)
        debug.print("   ▸  sampled accuracy error:     {:.3f}%", val_err)
